...
tab1, tab2, tab3 = st.tabs(["Paso 1: Incubadora", "Paso 2: Transporte", "Paso 3: Granja (Próximamente)"])
...
with tab1:
    ...
    with st.form("info_lote_form"):
        # ... (código del formulario de incubadora sin cambios) ...
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "patas_ok": st.column_config.CheckboxColumn("Patas OK?"),
                "ojos_ok": st.column_config.CheckboxColumn("Ojos OK?"),
        ...
        if submitted_incubadora:
            ...
            if not lote_id or not granja_origen or not evaluador:
                ...
                with st.spinner("Calculando resultados y guardando datos..."):
                    df = edited_df
                    puntuaciones = {
                        'vitalidad_ok': 15, 'ombligo_ok': 15, 'patas_ok': 4.75, 'ojos_ok': 4.75,
                    ...
                    resumen_data = [
                        ...
                        round(temp_cloacal_promedio, 2), round(puntuacion_final, 2), round(uniformidad, 2), round(cv_peso, 2)
                    ]
                    df_detalle = df.copy()
                    ...
                    columnas_detalle = ['lote_id', 'numero_pollito', 'vitalidad_ok', 'ombligo_ok', 'patas_ok', 'ojos_ok', 
                                        'pico_ok', 'abdomen_ok', 'plumon_ok', 'cuello_ok', 'peso_gr', 'temp_cloacal']
...
with tab2:
    ...
    with st.form("transporte_form"):
        ...
        with t_col3:
            comportamiento_llegada = st.selectbox("Comportamiento a la Llegada", 
                                                 ["Calmos y distribuidos uniformemente", 
                                                  "Ruidosos y agrupados (frío)", 
                                                  "Jadeando y apartados (calor)",
                                                  "Letárgicos o débiles"])
        ...
        if submitted_transporte:
            if not lote_id_transporte:
                ...
                with st.spinner("Guardando datos de transporte..."):
                    ...
                    if duracion < 0: duracion = 0 # En caso de error de fecha/hora

                    # Preparar datos para la fila
                    transporte_data = [
                        lote_id_transporte, str(fecha_transporte), placa_vehiculo, conductor,
                        str(hora_salida), str(hora_llegada), int(duracion),
                        float(temp_inicio), int(hum_inicio), float(temp_final), int(hum_final),
                        comportamiento_llegada, int(mortalidad_transporte)
                    ]
...
with tab3:
    ...
    st.image("https://i.imgur.com/vHq8S74.jpg", caption="La recepción en granja es el último paso crítico del arranque.")
